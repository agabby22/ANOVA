...
raw_data <- read_csv("~/Desktop/Dissertation/Analysis/raw_data/markdup.featurecounts.csv") %>%
...
design <- ~ dev_stage * treatment_length * treatment 

# Create a new DESeq object with raw data
...
normalized_counts <- counts(dds2, normalized = TRUE) %>% as.data.frame()

identifiers_2 <- getBM(values = rownames(dds2),
...
```{r}
#pulling out the "ensembl_gene_id" information 
normalized_counts <- rownames_to_column(normalized_counts, var = "ensembl_gene_id")
row.names(normalized_counts) <- 1:nrow(raw_data)
...
markers_normalized <- left_join(identifiers_2, normalized_counts, by = "ensembl_gene_id") %>%
...
markers_normalized <- markers_normalized %>% filter(hgnc_symbol %in% apoptosis_markers$hgnc_symbol) 

# Create an empty list to store the separate gene dataframes
gene_list <- list()

# Loop over each row
for (i in 1:nrow(markers_normalized)) {
  # Get the gene data
  gene <- markers_normalized[i, ]
...
column_names <- colnames(markers_normalized)[3:length(colnames(markers_normalized))]
...
for (gene in gene_dataframes) {
  gene_norm <- merge(gene, metadata, by = "Sample", all.x = TRUE)
  gene_norm[[2]] <- as.numeric(gene_norm[[2]])
  #add a column treatment + treatment_length using mutate()
  gene_norm <- gene_norm %>%
    mutate(term = paste0(dev_stage, ":", treatment_length))
  genes_norm <- c(genes_norm, list(gene_norm))
}
...
for (i in seq_along(genes_norm)) {
  gene_data <- genes_norm[[i]]
...
for (i in seq_along(anova_results_list)) {
  ...
  gene_name <- colnames(genes_norm[[i]])[2]
...
for (i in seq_along(anova_results_list)) {
  gene_name <- colnames(genes_norm[[i]])[2]  # Get the name of the second column (Counts column)
...
```{r}
#SAVE THE NORMALISED ANOVA RESULTS

setwd("~/Desktop/Dissertation/Analysis/Anova/files_for_anova/normalized/")

for (i in 1:length(genes_norm)) {
  second_column_name <- colnames(genes_norm[[i]])[2]
  file_name <- paste0(second_column_name, "_norm.csv")
  
  # Save the gene's data as a CSV file in the specified path
  write.csv(genes_norm[[i]], file = file_name, row.names = TRUE)
}

#SAVE ANOVA RESULTS

setwd("~/Desktop/Dissertation/Analysis/Anova/results/normalized")

# Assuming you already have the code for creating the 'anova_results_df_list'

# Loop through each dataframe in the list and save it as a separate Excel file
for (gene_name in names(anova_results_df_list)) {
  # Create the file name using the gene_name
  file_name <- paste(gene_name, "_anova_norm_res.xlsx", sep = "")
...
for (gene in genes_norm) {
  ...
  gene_res <- normalized_counts[normalized_counts$ensembl_gene_id == gene_id, ]
...
```{r}
#Saving the plots
setwd("~/Desktop/Dissertation/Analysis/Anova/plots/normalized/barplots/")

ggsave("norm_D7012h.png", barplot_D7012h)
ggsave("norm_D7024h.png", barplot_D7024h)
ggsave("norm_D10012h.png", barplot_D10012h)
ggsave("norm_D10024h.png", barplot_D10024h)
...
test$term <- factor(test$term, levels = c("D70:12h", "D70:24h", "D100:12h", "D100:24h"))
  ...
  p <- p + stat_pvalue_manual(stat.test,label = "{p.adj.signif}", hide.ns = TRUE, tip.length = 0.2, bracket.nudge.y = 0.4, step.increase = 1) 
...
setwd("~/Desktop/Dissertation/Analysis/Anova/Bonferroni/normalized")

for (i in seq_along(statistics)) {
  gene_name <- colnames(genes_norm[[i]])[2]
  file_name <- paste0(gene_name, "_norm.xlsx", sep = "")
  # Save the gene's data as a CSV file in the specified path
...
setwd("~/Desktop/Dissertation/Analysis/Anova/plots/normalized/padj_barplots/")
...
setwd("~/Desktop/Dissertation/Analysis/Anova/plots/normalized/combined/")

ggsave("combined_norm_plot1.png", combined_plots1)
ggsave("combined_norm_plot2.png", combined_plots2)
ggsave("combined_norm_plot3.png", combined_plots3)
